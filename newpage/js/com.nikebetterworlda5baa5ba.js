...
(function($) {
  ...
  $.fn.Video = function() {
     ...
     this.each(function() {
        ...
        $self
          .Scrollable({threshold: _threshold,is_nav:true,auto_scroll:false})
       ...
       if (!$.Mobile && !$.Unsupported) {
        $sections.each(
          function(i){
            ...
            // We don't add a link to the first section, and we
            // scroll to it immediately
            if(i == 0) {
              // Scroll directly to the first one
              _active = 0;
              _seek();
        ...
        $.Body
          ...
          // Fade the first navigation item in/out 3 times when page loaded


          setTimeout(function() {
            if(!$.browser.msie) {

              var $firstLiH1 = $("#main li:first h1"),
                  i = 0;

              function fadeInOut() {
                i++;

                $firstLiH1.stop().css({display:'block'}).animate({left:30,opacity:1}, 450, 'easeOutQuart', function() {
                  setTimeout(function() {
                    $firstLiH1.css({opacity: 0, left: 115 });
                    if(i >= 3) {
                      return;
                    } else {
                      setTimeout(function() {
