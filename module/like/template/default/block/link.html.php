...
<li class="li_action">
	...
	<a href="#" onclick="$(this).parents('div:first').find('.js_like_link_like:first').show(); $(this).hide(); $.ajaxCall('like.delete', 'type_id={$aLike.like_type_id}&amp;item_id={$aLike.like_item_id}&amp;parent_id={if isset($aFeed.feed_id)}{$aFeed.feed_id}{else}{/if}{if $aLike.like_is_custom}&amp;custom_inline=1{/if}', 'GET'); return false;" class="js_like_link_toggle js_like_link_unlike"{if $aLike.like_is_liked}{else} style="display:none;"{/if}><span class="font_icon">&#xf165;</span><!--{phrase var='feed.unlike'}--></a>	
</li>
